...
- it-pro
- goldenconfig
ms.openlocfilehash: 9a07dd54dac46bf3864e25c52a3cb745b22b5354
ms.sourcegitcommit: 485273c1679d30443c23366bbaadb53402287c20
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 11/27/2018
ms.locfileid: "26700668"
---
# <a name="policy-recommendations-for-securing-email"></a><span data-ttu-id="0dbb2-103">Richtlinienempfehlungen für sichere E-Mails</span><span class="sxs-lookup"><span data-stu-id="0dbb2-103">Policy recommendations for securing email</span></span>
<span data-ttu-id="0dbb2-p101">In diesem Artikel wird beschrieben, wie zum Implementieren der empfohlenen Identität und Gerät-Richtlinien zum Schutz der Organisationseinheit e-Mails und e-Mail-Clients, die moderne Authentifizierung und bedingte Zugriff zu unterstützen. Dieser Anleitung basiert auf den [gemeinsamen Identität und Gerät RAS-Richtlinien](identity-access-policies.md) und auch einige zusätzliche Informationen enthält.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p101">This article describes how to implement the recommended identity and device access policies to protect organizational email and email clients that support modern authentication and conditional access. This guidance builds on the [Common identity and device access policies](identity-access-policies.md) and also includes a few additional recommendations.</span></span>


<span data-ttu-id="0dbb2-p102">Diese Empfehlungen sind auf drei verschiedene Ebenen der Sicherheit und Schutz, die angewendet werden kann auf die Granularität von Ihren Anforderungen basieren: **Baseline**, **empfindlich**und **hochgradig regulierter**. Erfahren Sie mehr über diese Sicherheitskategorien und die empfohlenen Clientbetriebssysteme, die durch diesen Empfehlungen in der [Einführung in Security Policies und Konfigurationen empfohlen](microsoft-365-policies-configurations.md).</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p102">These recommendations are based on three different tiers of security and protection that can be applied based on the granularity of your needs: **baseline**, **sensitive**, and **highly regulated**. You can learn more about these security tiers, and the recommended client operating systems, referenced by these recommendations in the [recommended security policies and configurations introduction](microsoft-365-policies-configurations.md).</span></span>

<span data-ttu-id="0dbb2-p103">Diese Empfehlungen müssen die Benutzer bei modernen e-Mail-Clients, einschließlich Outlook für iOS und Android auf mobilen Geräten verwenden. Outlook für iOS und Android bieten Unterstützung für die besten Features von Office 365. Diese mobilen apps für Outlook sind auch mit der Sicherheit entworfen, Funktionen, die Unterstützung von mobilen verwenden und zusammen mit anderen Microsoft-Cloud-Sicherheitsfunktionen arbeiten. Weitere Informationen finden Sie unter [Outlook für iOS und Android häufig gestellte Fragen](https://docs.microsoft.com/exchange/clients-and-mobile-in-exchange-online/outlook-for-ios-and-android/outlook-for-ios-and-android-faq).</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p103">These recommendations require your users to use modern email clients, including Outlook for iOS and Android on mobile devices. Outlook for iOS and Android provide support for the best features of Office 365. These mobile Outlook apps are also architected with security capabilities that support mobile use and work together with other Microsoft cloud security capabilities. For more information, see [Outlook for iOS and Android FAQ](https://docs.microsoft.com/exchange/clients-and-mobile-in-exchange-online/outlook-for-ios-and-android/outlook-for-ios-and-android-faq).</span></span>

## <a name="updating-common-policies-to-include-email"></a><span data-ttu-id="0dbb2-112">Aktualisieren allgemeine Richtlinien zum Einbeziehen von e-Mail</span><span class="sxs-lookup"><span data-stu-id="0dbb2-112">Updating common policies to include email</span></span>
<span data-ttu-id="0dbb2-p104">Das folgende Diagramm veranschaulicht die allgemeine Identität und Gerät Zugriffsrichtlinien und gibt an, welche Richtlinien zum Schutz von e-Mail aktualisiert werden müssen. Beachten Sie das Hinzufügen einer neuen Regel für Exchange Online ActiveSync-Clients zu blockieren. Dies erzwingt die Verwendung von Outlook mobile.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p104">The following diagram illustrates the common identity and device access policies and indicates which policies need to be updated to protect email. Note the addition of a new rule for Exchange Online to block ActiveSync clients. This forces the use of Outlook mobile.</span></span>
...
<span data-ttu-id="0dbb2-p105">Wenn Sie Exchange Online und Outlook in den Bereich der Richtlinien enthalten, wenn Sie diese einrichten, müssen Sie nur die neue Richtlinie zum Blockieren von ActiveSync-Clients zu erstellen. Überprüfen Sie die in der folgenden Tabelle aufgeführten Richtlinien und stellen Sie die empfohlenen Ergänzungen oder zu bestätigen, dass diese bereits eingeschlossen sind. Jede Regel enthält Links zu den entsprechende konfigurationsanweisungen im Artikel [Allgemeine Zugriffsrichtlinien Identität und Gerät](identity-access-policies.md) .</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p105">If you included Exchange Online and Outlook in the scope of the policies when you set them up, you only need to create the new policy to block ActiveSync clients. Review the policies listed in the following table and either make the recommended additions, or confirm that these are already included. Each rule links to the associated configuration instructions in the [Common identity and device access policies](identity-access-policies.md) article.</span></span> 

|<span data-ttu-id="0dbb2-120">Schutzebene</span><span class="sxs-lookup"><span data-stu-id="0dbb2-120">Protection level</span></span>|<span data-ttu-id="0dbb2-121">Policies</span><span class="sxs-lookup"><span data-stu-id="0dbb2-121">Policies</span></span>|<span data-ttu-id="0dbb2-122">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="0dbb2-122">More information</span></span>|
|:---------------|:-------|:----------------|
|<span data-ttu-id="0dbb2-123">**Baseline**</span><span class="sxs-lookup"><span data-stu-id="0dbb2-123">**Baseline**</span></span>|[<span data-ttu-id="0dbb2-124">Erfordern Sie mehrstufiger Authentifizierung das, bei der Anmeldung Risiko *Mittel* oder *Hoch* ist</span><span class="sxs-lookup"><span data-stu-id="0dbb2-124">Require MFA when sign-in risk is *medium* or *high*</span></span>](identity-access-policies.md#require-mfa-based-on-sign-in-risk)|<span data-ttu-id="0dbb2-125">Schließen Sie Exchange Online in die Zuweisung von Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-125">Include Exchange Online in the assignment of cloud apps</span></span>|
|        |[<span data-ttu-id="0dbb2-126">Blockierung von Clients, die moderne Authentifizierung nicht unterstützen</span><span class="sxs-lookup"><span data-stu-id="0dbb2-126">Block clients that don't support modern authentication</span></span>](identity-access-policies.md#block-clients-that-dont-support-modern-authentication)|<span data-ttu-id="0dbb2-127">Schließen Sie Exchange Online in die Zuweisung von Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-127">Include Exchange Online in the assignment of cloud apps</span></span>|
|        |[<span data-ttu-id="0dbb2-128">Definieren von Richtlinien für die app-Schutz</span><span class="sxs-lookup"><span data-stu-id="0dbb2-128">Define app protection policies</span></span>](identity-access-policies.md#high-risk-users-must-change-password)|<span data-ttu-id="0dbb2-p106">Stellen Sie sicher, dass Outlook in der Liste der apps enthalten ist. Achten Sie zum Aktualisieren der Richtlinie für jede Plattform (iOS, Android, Windows)</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p106">Be sure Outlook is included in the list of apps. Be sure to update the policy for each platform (iOS, Android, Windows)</span></span>|
|        |[<span data-ttu-id="0dbb2-131">Genehmigte apps erfordern</span><span class="sxs-lookup"><span data-stu-id="0dbb2-131">Require approved apps</span></span>](identity-access-policies.md#require-approved-apps)|<span data-ttu-id="0dbb2-132">Einschließen von Exchange Online in der Liste der Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-132">Include Exchange Online in the list of cloud apps</span></span>|
|        |[<span data-ttu-id="0dbb2-133">Kompatible PCs erfordern</span><span class="sxs-lookup"><span data-stu-id="0dbb2-133">Require compliant PCs</span></span>](identity-access-policies.md#require-compliant-pcs-but-not-compliant-phones-and-tablets)|<span data-ttu-id="0dbb2-134">Einschließen von Exchange Online in der Liste der Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-134">Include Exchange Online in list of cloud apps</span></span>|
|        |[<span data-ttu-id="0dbb2-135">Block ActiveSync-clients</span><span class="sxs-lookup"><span data-stu-id="0dbb2-135">Block ActiveSync clients</span></span>](#block-activesync-clients)|<span data-ttu-id="0dbb2-136">Diese neue Richtlinie hinzufügen</span><span class="sxs-lookup"><span data-stu-id="0dbb2-136">Add this new policy</span></span>| 
|<span data-ttu-id="0dbb2-137">**Vertraulich**</span><span class="sxs-lookup"><span data-stu-id="0dbb2-137">**Sensitive**</span></span>|[<span data-ttu-id="0dbb2-138">Erfordern Sie mehrstufiger Authentifizierung das, bei der Anmeldung Risiko *Niedrig*, *Mittel* oder *Hoch* ist</span><span class="sxs-lookup"><span data-stu-id="0dbb2-138">Require MFA when sign-in risk is *low*, *medium* or *high*</span></span>](identity-access-policies.md#require-mfa-based-on-sign-in-risk)| <span data-ttu-id="0dbb2-139">Schließen Sie Exchange Online in die Zuweisung von Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-139">Include Exchange Online in the assignment of cloud apps</span></span>|
|         |[<span data-ttu-id="0dbb2-140">Erfordern Sie kompatible PCs *und* mobile Geräten</span><span class="sxs-lookup"><span data-stu-id="0dbb2-140">Require compliant PCs *and* mobile devices</span></span>](identity-access-policies.md#require-compliant-pcs-and-mobile-devices)|<span data-ttu-id="0dbb2-141">Einschließen von Exchange Online in der Liste der Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-141">Include Exchange Online in the list of cloud apps</span></span>|
|<span data-ttu-id="0dbb2-142">**Streng geregelt**</span><span class="sxs-lookup"><span data-stu-id="0dbb2-142">**Highly regulated**</span></span>|[<span data-ttu-id="0dbb2-143">*Immer* erfordern mehrstufiger Authentifizierung das</span><span class="sxs-lookup"><span data-stu-id="0dbb2-143">*Always* require MFA</span></span>](identity-access-policies.md#require-mfa-based-on-sign-in-risk)|<span data-ttu-id="0dbb2-144">Schließen Sie Exchange Online in die Zuweisung von Cloud-apps</span><span class="sxs-lookup"><span data-stu-id="0dbb2-144">Include Exchange Online in the assignment of cloud apps</span></span>|

## <a name="block-activesync-clients"></a><span data-ttu-id="0dbb2-145">Block ActiveSync-clients</span><span class="sxs-lookup"><span data-stu-id="0dbb2-145">Block ActiveSync clients</span></span>
<span data-ttu-id="0dbb2-p107">Diese Richtlinie verhindert, dass ActiveSync-Clients und andere bedingte Zugriffsregeln umgehen. Die Konfiguration der Regel gilt nur für ActiveSync-Clients. Durch Auswählen von **Client-app genehmigt erfordern**, blockiert diese Richtlinie ActiveSync-Clients. So konfigurieren Sie diese Richtlinie:</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p107">This policy prevents ActiveSync clients from bypassing other conditional access rules. The rule configuration applies only to ActiveSync clients. By selecting **Require approved client app**, this policy blocks ActiveSync clients. To configure this policy:</span></span>

1. <span data-ttu-id="0dbb2-p108">Wechseln Sie zu der [Azure-Portal](https://portal.azure.com), und melden Sie sich mit Ihren Anmeldeinformationen. Nachdem Sie sich erfolgreich angemeldet haben, sehen Sie das Dashboard Azure.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p108">Go to the [Azure portal](https://portal.azure.com), and sign in with your credentials. After you've successfully signed in, you see the Azure dashboard.</span></span>

2. <span data-ttu-id="0dbb2-152">Wählen Sie im linken Menü **Azure Active Directory** aus.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-152">Choose **Azure Active Directory** from the left menu.</span></span>

3. <span data-ttu-id="0dbb2-153">Klicken Sie im Bereich **Sicherheit** auf **Bedingter Zugriff**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-153">Under the **Security** section, choose **Conditional access**.</span></span>

4. <span data-ttu-id="0dbb2-154">Wählen Sie **Neue Richtlinie** aus.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-154">Choose **New policy**.</span></span>

5. <span data-ttu-id="0dbb2-155">Geben Sie einen Richtliniennamen ein, und wählen Sie dann die **Benutzer und Gruppen** aus, auf die Sie die Richtlinie anwenden möchten.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-155">Enter a policy name, then choose the **Users and groups** you want to apply the policy for.</span></span>

6. <span data-ttu-id="0dbb2-156">Klicken Sie auf **Cloud-Apps**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-156">Choose **Cloud apps**.</span></span>

7. <span data-ttu-id="0dbb2-p109">Wählen Sie **apps auswählen**, wählen Sie **Office 365 Exchange Online**. Die Option **auswählen** und **durchgeführt**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p109">Choose **Select apps**, select **Office 365 Exchange Online**. Choose **Select** and **Done**.</span></span>

8. <span data-ttu-id="0dbb2-159">Wählen Sie die **Bedingungen**, und wählen Sie die **Client-apps**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-159">Choose **Conditions**, and then choose **Client apps**.</span></span>

9. <span data-ttu-id="0dbb2-p110">Wählen Sie für **Konfigurieren** **Ja**aus. Überprüfen Sie nur die folgenden: **mobiler apps und Desktopclients** und **Exchange ActiveSync-Clients**. Wählen Sie **Fertig**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p110">For **Configure**, select **Yes**. Check only the following: **Mobile apps and desktop clients** and **Exchange ActiveSync clients**. Choose **Done**.</span></span>

10. <span data-ttu-id="0dbb2-163">Wählen Sie im Abschnitt **Zugriffssteuerung** die Option **Erteilen** aus.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-163">Choose **Grant** from the **Access controls** section.</span></span>

11. <span data-ttu-id="0dbb2-p111">Wählen Sie **Zugriff gewähren**, wählen Sie **Client-app genehmigt erforderlich**.  Wählen Sie für mehrere Steuerelemente **die ausgewählten Steuerelemente erforderlich**, und klicken Sie dann **auf auswählen**.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p111">Choose **Grant access**, select **Require approved client app**.  For multiple controls, select **Require the selected controls**, then choose **Select**.</span></span> 

12. <span data-ttu-id="0dbb2-166">Wählen Sie **Create** aus.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-166">Choose **Create**.</span></span>

## <a name="setup-office-365-message-encryption"></a><span data-ttu-id="0dbb2-167">Einrichten von Office 365-nachrichtenverschlüsselung</span><span class="sxs-lookup"><span data-stu-id="0dbb2-167">Setup Office 365 message encryption</span></span>
<span data-ttu-id="0dbb2-p112">Mit den neuen Funktionen von Office 365 Message Encryption (OME), die die Schutzfunktionen in Azure Information Protection nutzen, kann in Ihrer Organisation auf einfache Weise geschützte e-Mails mit jedem beliebigen Gerät freigeben. Benutzer können Nachrichten senden und empfangen geschützte mit anderen Office 365-Organisationen sowie nicht - Office 365-Kunden mit Outlook.com, Google Mail und anderen e-Mail-Dienste.</span><span class="sxs-lookup"><span data-stu-id="0dbb2-p112">With the new Office 365 Message Encryption (OME) capabilities, which leverage the protection features in Azure Information Protection, your organization can easily share protected email with anyone on any device. Users can send and receive protected messages with other Office 365 organizations as well as non-Office 365 customers using Outlook.com, Gmail, and other email services.</span></span>

<span data-ttu-id="0dbb2-170">Weitere Informationen finden Sie unter [Einrichten von neuen Funktionen von Office 365 Message Encryption](https://support.office.com/article/set-up-new-office-365-message-encryption-capabilities-7ff0c040-b25c-4378-9904-b1b50210d00e).</span><span class="sxs-lookup"><span data-stu-id="0dbb2-170">For more information, see [Set up new Office 365 Message Encryption capabilities](https://support.office.com/article/set-up-new-office-365-message-encryption-capabilities-7ff0c040-b25c-4378-9904-b1b50210d00e).</span></span> 
...
## <a name="next-steps"></a><span data-ttu-id="0dbb2-171">Nächste Schritte</span><span class="sxs-lookup"><span data-stu-id="0dbb2-171">Next steps</span></span>

[<span data-ttu-id="0dbb2-172">Weitere Informationen zu Richtlinienempfehlungen zum Schutz von SharePoint-Websites und -Dokumenten</span><span class="sxs-lookup"><span data-stu-id="0dbb2-172">Learn about policy recommendations for securing SharePoint Sites and files</span></span>](sharepoint-file-access-policies.md)
