...
3. Suchen Sie in der Ergebnisliste in der Zeile **AMRunningMode** nach einem der folgenden Werte:
   - `Normal` – Der Defender-Dienst wird normal ausgeführt. Es sind keine speziellen Modi aktiviert.
   - `Passive Mode`– Wenn Ihre Organisation Microsoft Defender for Endpoint zusammen mit einer Nicht-Microsoft-Antiviren-/Antischalwarelösung verwendet, wechselt Microsoft Defender Antivirus automatisch in den passiven Modus. (Echtzeitschutz und Bedrohungen werden nicht von der Microsoft Defender Antivirus.)
   - `SxS Passive Mode`- Ähnlich wie im passiven Modus, aber mit der Option zum Aktivieren begrenzter periodischer Überprüfungen.
