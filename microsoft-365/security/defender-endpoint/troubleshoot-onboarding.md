...
- Sie können auch die vorherigen Registrierungsschlüsselwerte überprüfen, um zu überprüfen, ob die Richtlinie deaktiviert ist, indem Sie den Registrierungsschlüssel `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows Defender` öffnen.
   ...
   > [!NOTE]
   > Darüber hinaus müssen Sie sicherstellen, dass wdfilter.sys und wdboot.sys auf die Standardwerte "0" festgelegt sind.
   >
...
- [Sicherstellen, dass Microsoft Monitoring Agent (MMA) installiert und konfiguriert ist, um Sensordaten an den Dienst zu melden](configure-server-endpoints.md#server-mma)
- [Stellen Sie sicher, dass die Serverproxy- und Internetverbindungseinstellungen ordnungsgemäß konfiguriert sind.](configure-server-endpoints.md#server-proxy)
...
- [Problembehandlung für Microsoft Defender for Endpoint](troubleshoot-mdatp.md)
