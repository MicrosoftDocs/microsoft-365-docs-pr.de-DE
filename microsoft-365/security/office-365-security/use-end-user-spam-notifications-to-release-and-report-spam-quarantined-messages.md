...
- **Größe**: die Größe der isolierten Nachricht in Kilobyte (KBS).

Sie können folgende Aktionen für jede Nachricht ausführen:

- Im **Posteingang freigeben**: Wenn Sie auf diesen Link klicken, wird die Nachricht an Ihren Posteingang gesendet, wo Sie Sie anzeigen können.

- **Bericht als nicht-Junk**: Wenn Sie auf diesen Link klicken, wird eine Kopie der Nachricht zur Analyse an Microsoft gesendet. Das Spamteam bewertet und analysiert die Nachricht und passt, je nach Analyseergebnis, die Antispamfilterregeln so an, dass die Nachricht übergeben wird.

> [!NOTE]
> Nachrichten, die aufgrund einer e-Mail-Fluss Regel (auch bekannt als a) unter Quarantäne gestellt werden, sind nicht in in Quarantäne gestellten Spamnachrichten für Endbenutzer enthalten. Es werden nur Spamquarantäne-Nachrichten aufgeführt. <br/><br/>  Sie können eine Nachricht nur einmal freigeben und als falsch positiv markiert (keine Junk-E-Mail) melden.
