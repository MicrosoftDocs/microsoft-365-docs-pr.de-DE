...
|Empfängerbenachrichtigungen|Isolieren Sie die Nachricht im Hintergrund, oder isolieren Sie die Nachricht, und stellen Sie sie auch mit allen Anlagen in Quarantäne, die durch eine einzelne Textdatei mit Standard- oder benutzerdefiniertem Text ersetzt werden. Weitere Informationen finden Sie unter ["Konfigurieren von An malware-Richtlinien".](configure-anti-malware-policies.md)|
|Allgemeine Anlagenfilterung|Sie können eine Liste von Dateitypen aktivieren und anpassen, bei denen immer davon ausgegangen wird, dass es sich um Schadsoftware handelt. Weitere Informationen finden Sie unter [Schutz vor Schadsoftware in EOP](anti-malware-protection.md).|
...
|Richtlinienregeln pro Domäne|Der Bereich einer Nachrichtenflussregel kann so angepasst werden, dass er den Absender- oder Empfängerdomänennamen, IP-Adressbereichen, Adressschlüsselwörtern oder -mustern, Gruppenmitgliedschaften und anderen Bedingungen zupasst.|
