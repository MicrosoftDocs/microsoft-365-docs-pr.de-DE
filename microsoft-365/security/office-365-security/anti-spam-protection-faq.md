...
**Für eingehende Nachrichten:** Der Großteil des Spams wird über die Verbindungsfilterung gelöscht, die auf der IP-Adresse des Quell-E-Mail-Servers basiert. Antispamrichtlinien (auch als Spamfilterrichtlinien oder Inhaltsfilterrichtlinien bezeichnet) überprüfen und klassifizieren Nachrichten als Spam, Massen- oder Phishing. Nachrichten, die als Spam oder Massennachrichten klassifiziert wurden, werden standardmäßig an den Junk-E-Mail-Ordner des Empfängers zugestellt, während Nachrichten, die als Phishing klassifiziert wurden, unter Quarantäne gestellt werden. Sie können die standardmäßige Antispamrichtlinie ändern (gilt für alle Empfänger), oder Sie können benutzerdefinierte Antispamrichtlinien mit strengeren Einstellungen für bestimmte Benutzergruppen erstellen (sie können z. B. Spam isolieren, der an Führungskräfte gesendet wird). Weitere Informationen finden Sie unter ["Konfigurieren von Antispamrichtlinien"](configure-your-spam-filter-policies.md) und ["Empfohlene Antispamrichtlinieneinstellungen".](recommended-settings-for-eop-and-office365-atp.md#eop-anti-spam-policy-settings)
 ...
 **Für ausgehende Nachrichten:** Die Nachricht wird entweder [](high-risk-delivery-pool-for-outbound-messages.md) über den Pool für besonders riskante Zustellnachrichten geroutet oder in einem Unzustellbarkeitsbericht (auch als NDR oder Unzustellbarkeitsnachricht bekannt) an den Absender zurückgesendet. Weitere Informationen zum Schutz vor ausgehenden Spamnachrichten finden Sie unter ["Ausgehende Spamsteuerelemente".](outbound-spam-controls.md)
...
Eine Zero-Day-Spamvariante ist eine bisher unbekannte Spamvariante der ersten Generation, die nie erfasst oder analysiert wurde. Daher stehen unseren Antispamfiltern noch keine Informationen zur Verfügung, um sie zu erkennen. Nachdem ein Zero-Day-Spambeispiel von unseren Spamanalysten erfasst und analysiert wurde und die Kriterien für die Spamklassifizierung erfüllt, werden unsere Antispamfilter aktualisiert, um sie zu erkennen, und sie werden nicht mehr als "Zero-Day" betrachtet.

**Hinweis:** Wenn Sie eine Nachricht erhalten, die eine Zero-Day-Spamvariante sein kann, um uns bei der Verbesserung des Diensts zu helfen, senden Sie die Nachricht mithilfe einer der unter "Melden von Nachrichten und Dateien" beschriebenen Methoden an [Microsoft.](report-junk-email-messages-to-microsoft.md)
...
Ja, mit dem Tool für die Nachrichtenverfolgung können Sie E-Mail-Nachrichten verfolgen, während sie den Dienst passieren, um herauszufinden, was mit ihnen passiert ist. Weitere Informationen zur Verwendung des Tools für die Nachrichtenverfolgung, um herauszufinden, warum eine Nachricht als Spam gekennzeichnet wurde, finden Sie unter "Wurde eine Nachricht als Spam [markiert?](https://docs.microsoft.com/exchange/monitoring/trace-an-email-message/message-trace-faq#was-a-message-marked-as-spam)
...
- **Der E-Mail-Server für ausgehende E-Mails sollte über einen Reverse-DNS-Eintrag (PTR) verfügen.**
  ...
  Wenn die E-Mail-Quell-IP-Adresse beispielsweise "192.0.43.10" ist, wäre der umgekehrte DNS-Eintrag `43-10.any.icann.org` "."
...
- **Wenn Sie eine Massensendung an zahlreiche Empfänger senden und die Nachricht ein Newsletterformat aufweist, sollte unten in der Nachricht mitgeteilt werden, wie der Newsletter abbestellt werden kann.**
  ...
  > Diese Nachricht wurde von "sender@fabrikam.com" an "example@contoso.com" geschickt. Profil-/E-Mail-Adressinformationen | Sofortige Entfernung mit **SafeUnsubscribe** &trade; | Datenschutzrichtlinie
...
- **Formatieren ausgehender Unzustellbarkeitsnachrichten.**
  ...
  Beim Generieren von Benachrichtigungen über den Zustellungsstatus (auch als Unzustellbarkeitsberichte, Unzustellbarkeitsberichte oder Unzustellbarkeitsnachrichten bezeichnet) sollten Absender das Format einer Unzustellbarkeitsmeldung wie in [RFC 3464 angegeben](https://www.ietf.org/rfc/rfc3464.txt)verwenden.
