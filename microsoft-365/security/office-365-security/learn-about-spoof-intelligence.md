...
4. Wählen Sie **im angezeigten Flyout** "Entscheiden, ob diese Absender Ihre Benutzer spoofen dürfen" eine der folgenden Registerkarten aus:
...
5. Klicken ![ Sie in der Spalte ](../../media/scc-expand-icon.png) **"Spoofing** zugelassen" auf das Symbol "Erweitern". Wählen **Sie "Ja"** aus, um den gefälschten Absender zu erlauben, oder wählen Sie **"Nein"** aus, um die Nachricht als spoofing zu markieren. Die Aktion wird durch die standardmäßige Antiphishingrichtlinie oder benutzerdefinierte Antiphishingrichtlinien gesteuert (der Standardwert ist "Nachricht in Junk-E-Mail-Ordner **verschieben").** Weitere Informationen finden Sie unter ["Spoofing"-Einstellungen in Antiphishingrichtlinien.](set-up-anti-phishing-policies.md#spoof-settings)
   ...
   - **Gefälschter Benutzer:** Das Benutzerkonto, das gefälscht wird. Dies ist der Absender der Nachricht in der Absenderadresse (auch als Adresse bezeichnet), die `5322.From` in E-Mail-Clients angezeigt wird. Die Gültigkeit dieser Adresse wird nicht von SPF überprüft.

     - Auf der **Registerkarte "Ihre Domänen"** enthält der Wert eine einzelne E-Mail-Adresse, oder wenn der Quell-E-Mail-Server mehrere Benutzerkonten spoofing, enthält er **mehrere**.
...
## <a name="use-the-security--compliance-center-to-configure-spoof-intelligence"></a>Verwenden des Security & Compliance Center zum Konfigurieren von Spoofing Intelligence
...
Um zu überprüfen, ob Sie spoofing intelligence mit Absendern konfiguriert haben, die spoofing dürfen und nicht spoofing dürfen, und ob Sie die Einstellungen für spoofing intelligence konfiguriert haben, verwenden Sie einen der folgenden Schritte:

- Wechseln Sie im Security & Compliance  Center zu Richtlinie zur Bedrohungsverwaltung Antispam erweitern \>  \>  \> **Spoofing Intelligence-Richtlinie.** \>  \>    Wählen Sie "Absender anzeigen" aus, die ich bereits überprüft habe, wählen Sie die Registerkarte "Ihre Domänen" oder "Externe Domänen" aus, und überprüfen Sie den Wert "Spoofing zugelassen".
...
- Wechseln Sie im Security & Compliance Center zu "Threat **Management** \> **Policy** \> **Anti-phishing"** oder **"ATP anti-phishing",** und gehen Sie wie folgt vor:  
