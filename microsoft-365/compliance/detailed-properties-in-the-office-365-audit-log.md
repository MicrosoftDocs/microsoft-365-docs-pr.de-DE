...
|ClientInfoString|Informationen zum E-Mail-Client, der zum Ausführen des Vorgangs verwendet wurde, z. B. eine Browserversion, Outlook version und Informationen zu mobilen Geräten|Exchange (Postfachaktivität)|
...
|ObjectId|Für Exchange-Verwaltungsüberwachungsprotokolle der Name des Objekts, das vom Cmdlet geändert wurde.  <br/> Für SharePoint den vollständigen URL-Pfadnamen der Datei oder des Ordners, auf die ein Benutzer zugegriffen hat.  <br/> Bei Azure AD-Aktivitäten der Name des geänderten Benutzerkontos.|Alle|
...
|Parameter|Für Exchange Administratoraktivität den Namen und Wert für alle Parameter, die mit dem Cmdlet verwendet wurden, das in der Operation-Eigenschaft identifiziert ist.|Exchange (Administratoraktivität)|
...
|ResultStatus|Gibt an, ob die Aktion (angegeben in der **Operation-Eigenschaft)** erfolgreich war oder nicht.  <br/> Für Exchange Administratoraktivität ist der Wert **entweder True** (erfolgreich) oder **False** (fehlgeschlagen).|Alle  <br/>|
...
|Ziel|Der Benutzer, für den die Aktion (in der **Operation-Eigenschaft** identifiziert) ausgeführt wurde. Wenn z. B. ein Gastbenutzer zu SharePoint oder einem Microsoft Team hinzugefügt wird, wird dieser Benutzer in dieser Eigenschaft aufgeführt.|Azure Active Directory|
...
|Arbeitslast|Der Microsoft 365, in dem die Aktivität aufgetreten ist.|Alle|
||||

> [!NOTE]
><sup>1</sup> Für Azure Active Directory ereignisse wird der Wert für einen Administrator nicht in einem Überwachungsdatensatz verwendet. Überwachungsdatensätze für Aktivitäten, die von Administratoren ausgeführt werden, geben an, dass ein regulärer Benutzer (z. B. **UserType: 0**) die Aktivität ausgeführt hat. Die **UserID-Eigenschaft** identifiziert die Person (regulärer Benutzer oder Administrator), die die Aktivität ausgeführt hat.<br/>
